...
#define SERVER_TIMEOUT_WARNING_REMAINING_SEC		30
#define SERVER_SESSION_ACTIVITY_TIMEOUT_SEC			120	// 2 min for testing, MUST be > SERVER_TIMEOUT_WARNING_REMAINING_SEC
...
ServerLobbyThread::HandlePacket(boost::shared_ptr<SessionData> session, boost::shared_ptr<NetPacket> packet)
{
	if (session && packet) {
		if (packet->IsClientActivity()) {
			LOG_ERROR("[AFK-TIMER] Session #" << session->GetId() << " client activity: msgtype=" << packet->GetMsg()->messagetype() << " -> resetting timer");
...
{
	LOG_ERROR("[AFK-TIMER] SessionTimeoutWarning for session #" << session->GetId() << " - sending TimeoutWarningMessage with " << remainingSec << "s remaining");
	boost::shared_ptr<NetPacket> packet(new NetPacket);
